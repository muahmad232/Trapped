...
enum GameState { START, MENU, DIFFICULTY_SELECT, PLAYING, MULTIPLAYER_PLAYING, SCORES };
...
class Button {
    ...
    sf::Text text;
    int defaultAlpha; // Default transparency
    int hoverAlpha;   // Transparency when hovered

public:
    // Constructor for initializing the button
    Button(const std::string& str, const sf::Font& font, float x, float y, int defaultAlpha = 150, int hoverAlpha = 255) {
        shape.setSize(sf::Vector2f(350, 100)); // Default button size
        shape.setPosition(x, y);
        this->defaultAlpha = defaultAlpha;
        this->hoverAlpha = hoverAlpha;

        // Set default color with transparency
        shape.setFillColor(sf::Color(0, 0, 0, defaultAlpha));

        text.setString(str);
        text.setFont(font);
        text.setCharacterSize(24); // Default text size
        text.setFillColor(sf::Color(192, 192, 192)); // Default text color
    ...
    // Function to set the transparency of the button
    void setTransparency(int alpha) {
        ...
        shape.setFillColor(sf::Color(currentColor.r, currentColor.g, currentColor.b, alpha));
    }
};
...
private:
    ...
    bool showHint = false;
    sf::Clock hintTimer;
    Button startButton{ "Start", font, 300, 600 };
...
public:
        ...
        hintButton("Hint", font, 10, 120) // Adjusted position for the hint button
    {
        ...
        if (!startBackgroundTexture.loadFromFile("start_background.jpg")) {
            throw std::runtime_error("Could not load start background image");
        }
    ...
    void handleEvents() {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (state == START && event.type == sf::Event::MouseButtonPressed) {
                    sf::Vector2i mousePos = sf::Mouse::getPosition(window);
                    if (startButton.contains(mousePos)) {
                        state = MENU; // Transition to the main menu
                    }
                }
